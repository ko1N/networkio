...
inline uint64_t file_size(const std::string &name) {
  std::ifstream f(name.c_str(), std::ifstream::ate | std::ifstream::binary);
  return (uint64_t)f.tellg();
}

inline std::string file_basename(const std::string &name) {
...
template <typename T> inline std::shared_ptr<T> make_shared_array(size_t size) {
...
public:
  ...
  void add_queue(chunk_t *chunk) { this->m_queue.push_back(chunk); }

  size_t queue_size() { return this->m_queue.size(); }
...
void worker_allocate(std::vector<uint32_t> ports) {
...
int main(int argc, char *argv[]) {
  ...
  client->event(networkio::mux::event_type::connect,
  ...
  client->endpoint(
      HASH("sendfile/start"),
      [&](networkio::mux::client *client, networkio::proto::packet *packet) {
