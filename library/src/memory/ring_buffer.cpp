...
ring_buffer::ring_buffer(int32_t initial_size) {
  this->_ensure_capacity(initial_size);
}
...
void ring_buffer::push_back(const uint8_t *buf, int32_t size) {
...
int32_t ring_buffer::pop_front(uint8_t *buf, int32_t size) {
  std::lock_guard<std::mutex> lock(this->m_mutex);
  return this->_pop_front(buf, size);
}

int32_t ring_buffer::pop_front(int32_t size) {
  std::lock_guard<std::mutex> lock(this->m_mutex);
...
auto ring_buffer::size() -> int32_t {
  std::lock_guard<std::mutex> lock(this->m_mutex);
  return this->_bytes_written();
}

auto ring_buffer::_bytes_left() const -> int32_t {
  return this->m_buffer_size - this->_bytes_written();
}

auto ring_buffer::_bytes_written() const -> int32_t {
  return this->m_bytes_written;
}

void ring_buffer::_ensure_capacity(int32_t size) {
  if (this->m_buffer == nullptr) {
    this->m_buffer = (uint8_t *)malloc(size);
    this->m_buffer_size = size;
  } else {
    this->m_buffer =
        (uint8_t *)realloc(this->m_buffer, this->m_buffer_size + size);
    this->m_write_offs += this->m_buffer_size;
    this->m_buffer_size += size;
  }
}

int32_t ring_buffer::_pop_front(uint8_t *buf, int32_t size) {
  int32_t rcvd = std::min(size, this->_bytes_written());
  if (rcvd <= 0) {
    return 0;
  }
