...
const uint8_t *buffer::data() const { return this->m_begin; }

const uint32_t buffer::size() const { return this->m_size - this->m_offset; }

void buffer::ensure_capacity(uint32_t size) {
  if (this->m_buffer == nullptr) {
    this->m_buffer = (uint8_t *)malloc(size);
...
void buffer::set_offset(uint32_t offset) {
