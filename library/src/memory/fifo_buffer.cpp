...
void fifo_buffer::push_back(const uint8_t *buf, int32_t size) {
  if (size <= 0) {
    return;
  }
...
auto fifo_buffer::pop_front(uint8_t *buf, int32_t size) -> int32_t {
  if (size <= 0) {
    return 0;
  }
...
auto fifo_buffer::pop_front(int32_t size) -> int32_t {
  if (size <= 0) {
    return 0;
  }
...
auto fifo_buffer::_pop_front(int32_t size) -> int32_t {
  int32_t rcvd = std::min(size, this->m_buffer_size);
  if (rcvd <= 0) {
    return 0;
  }
  ...
  if (rcvd == this->m_buffer_size) {
    ...
    auto *new_buffer = (uint8_t *)malloc(new_buffer_size);
