...
packet_proto::read_packet() {
	...
	if (this->m_in_pkt_header.header.magic == this->pkt_header_magic &&
		(uint32_t)this->m_in_buffer.size() >= this->m_in_pkt_header.header.pkt_size) {
		auto buf = std::make_shared<memory::buffer>(this->m_in_pkt_header.header.pkt_size);
		this->m_in_buffer.pop_front(const_cast<uint8_t *>(buf->data()), this->m_in_pkt_header.header.pkt_size);
		this->m_in_pkt_header.header.magic = 0; // reset header
	...
	if (!this->m_client->is_stream()) {
		...
		if (!this->is_valid_pkt_header(hdr) || hdr->header.pkt_size != (uint32_t)read - sizeof(pkt_header_t)) {
...
packet_proto::process() {
	...
	if (this->m_out_buffer == nullptr) {
		...
		this->m_out_buffer = std::make_shared<memory::buffer>((uint32_t)sizeof(pkt_header_t) + p->size());
		...
		memcpy(const_cast<uint8_t *>(this->m_out_buffer->data() + sizeof(pkt_header_t)), p->buffer()->data(),
			   p->size());
	...
	if (this->m_out_buffer != nullptr) {
		int32_t sent =
			this->m_client->write_raw(const_cast<uint8_t *>(this->m_out_buffer->data()) + this->m_out_data_sent,
...
bool
packet_proto::is_valid_pkt_header(const pkt_header_t *hdr) {
	return (hdr->header.magic == this->pkt_header_magic && hash::fnv1a(&hdr->header, sizeof(hdr->header)) == hdr->hash);
}
