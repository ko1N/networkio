...
packet::packet(networkio::memory::shared_buffer buffer, SOCKADDR_IN *address) {
  ...
  this->m_write_pointer =
      const_cast<uint8_t *>(this->m_buffer->data()) + this->m_buffer->size();
...
uint32_t packet::read_bytesleft() const {
...
void packet::ensure_capacity(uint32_t length) {
  if (this->m_buffer == nullptr) {
    ...
    if ((this->m_buffer->size() -
         (uint32_t)(this->m_write_pointer - this->m_buffer->data())) < length) {
      ...
      this->m_write_pointer =
          const_cast<uint8_t *>(this->m_buffer->data()) + old_size;
...
void packet::write_string(const std::string &str) {
