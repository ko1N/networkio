...
bool server::set_certificate(const std::string &cert,
                             const std::string &priv_key,
                             const std::string &priv_key_pwd) {
  this->m_credential_manager.reset(
      new credentials_manager(cert, this->rng(), priv_key, priv_key_pwd));
  return true;
}

bool server::set_buffer_size(size_t bufsize) {
  this->m_bufsize = bufsize;
  return true;
}

// TODO: this is not the best design and we can probably get rid of this func
bool server::create_socket() { return false; }

bool server::set_blocking(bool block) {
  if (this->m_server == nullptr)
    return false;
...
bool server::set_nopipe(bool nopipe) {
  if (this->m_server == nullptr)
    return false;
...
bool server::bind(const u_short port) {
...
Botan::RandomNumberGenerator &server::rng() {
