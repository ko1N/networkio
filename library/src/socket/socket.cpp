...
int
networkio::socket::sockaddr_cmp(struct sockaddr *x, struct sockaddr *y) {
#define CMP(a, b)                                                                                                      \
	if (a != b)                                                                                                        \
	return a < b ? -1 : 1
...
socket::set_blocking(bool blocking) {
	...
	if (fcntl(this->m_sockfd, F_SETFL, (blocking ? (fl & ~O_NONBLOCK) : (fl | O_NONBLOCK))) >= 0) {
		this->m_blocking = blocking;
		return true;
	}
#endif
...
socket::set_nopipe(bool nopipe) {
	...
	int p = nopipe ? 1 : 0;
	if (setsockopt(this->m_sockfd, SOL_SOCKET, SO_NOSIGPIPE, (void *)&p, sizeof(p)) < 0) {
		return false;
	}
#endif
