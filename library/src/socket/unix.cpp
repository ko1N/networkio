...
bool unix_socket::create_socket() {
...
bool unix_server::bind(const u_short port) {
  this->create_socket();
  if (this->m_sockfd == INVALID_SOCKET) {
    return false;
  }
  ...
  strncpy(serv_addr.sun_path, this->m_path.c_str(),
  ...
  if (::bind(this->m_sockfd, (struct sockaddr *)&serv_addr,
             SUN_LEN(&serv_addr)) == SOCKET_ERROR)
    return false;
...
bool unix_client::connect(const std::string &addr) {
  this->create_socket();
  if (this->m_sockfd == INVALID_SOCKET) {
    return false;
  }
  ...
  int error = ::connect(this->m_sockfd, (struct sockaddr *)&serv_addr,
                        SUN_LEN(&serv_addr));
...
bool unix_client::is_connected() { return this->m_connected; }

bool unix_client::close() {
  this->m_connected = false;
  return unix_socket::close();
}

int unix_client::read_raw(uint8_t *buffer, int32_t size) {
...
int unix_client::write_raw(const uint8_t *buffer, int32_t size) {
  if (!this->is_connected()) {
    return -1;
  }
