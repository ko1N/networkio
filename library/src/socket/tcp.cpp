...
bool
tcp_server::bind(const u_short uPort) {
	this->create_socket();
	if (this->m_sockfd == INVALID_SOCKET)
		return false;
...
tcp_server::accept(struct sockaddr_in *addr) {
	...
	if (!sock->set_blocking(this->m_blocking) || !sock->set_nopipe(this->m_nopipe)) {
		return nullptr;
	}
...
bool
tcp_client::write_raw_blocked(const uint8_t *pBuffer, int32_t nSize) {
	int32_t nBytesLeft = nSize;
	while (nBytesLeft > 0) {
		if (this->can_send() <= 0)
			continue;

		int sent = send(this->m_sockfd, (char *)pBuffer, nBytesLeft, this->_flags());
		if (sent <= 0) {
			this->m_connected = false;
			return false;
		}
...
int32_t
tcp_client::read_raw(uint8_t *buf, int32_t len) {
...
*/
int32_t
tcp_client::write_raw(const uint8_t *buf, int32_t len) {
	if (!this->is_connected()) {
		return -1;
	}
