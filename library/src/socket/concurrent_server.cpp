...
bool
concurrent_server::set_sleep(uint32_t sleep) {
	this->m_sleep = sleep;
	return true;
}
...
bool
concurrent_server::set_threads(uint32_t threads) {
...
bool
concurrent_server::run(u_short port) {
	if (!this->start(port)) {
		return false;
	}
...
bool
concurrent_server::start(u_short port) {
	if (this->m_server->bind(port)) {
		...
		for (uint32_t i = 0; i < this->m_threads; i++) {
			this->m_threadpool.push_back(std::thread(concurrent_server::worker, this));
		}
		return true;
	}
...
void
concurrent_server::worker(concurrent_server *sv) {
