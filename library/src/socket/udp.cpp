...
bool udp_server::bind(const u_short port) {
  this->create_socket();
  if (this->m_sockfd == INVALID_SOCKET)
    return false;
...
udp_server::accept(struct sockaddr_in *addr) {
  ...
  if (addr != nullptr) {
    memcpy(addr, &this->m_new_clients.front().first,
...
void udp_server::_recv() {
  ...
  int rcvd = ::recvfrom(this->m_sockfd, (char *)buf, buf_size, 0,
                        (SOCKADDR *)&client_addr, &from_size);
  ...
  for (size_t i = 0; i < this->m_clients.size(); i++) {
    if (!sockaddr_cmp((struct sockaddr *)&std::get<0>(this->m_clients[i]),
  ...
  if (new_client) {
    ...
    auto c =
        std::pair<SOCKADDR_IN, std::shared_ptr<udp_client>>(client_addr, cl);
...
udp_client::udp_client(udp_server *server, SOCKET sockfd, SOCKADDR_IN *addr)
    : udp_socket() {
...
bool udp_client::connect(const std::string &addr) {
  this->create_socket();
  if (this->m_sockfd == INVALID_SOCKET) {
    return false;
  }
  ...
  int sent = ::sendto(this->m_sockfd, nullptr, 0, 0, (SOCKADDR *)&a,
                      sizeof(SOCKADDR_IN));
  if (sent < 0) {
    return false;
  }
...
bool udp_client::is_connected() { return this->m_connected; }
...
int32_t udp_client::read_raw(uint8_t *buf, int32_t size) {
  if (this->m_should_recv) {
    if (this->can_recv() <= 0) {
      return 0;
    }
    ...
    int rcvd = ::recvfrom(this->m_sockfd, (char *)buf, size, 0,
                          (SOCKADDR *)&server_addr, &from_size);
    if (rcvd < 0 || rcvd >= NET_MAX_MESSAGE) {
      if (rcvd < 0 && !this->m_blocking && errno == EAGAIN) {
        return 0;
      }
...
int32_t udp_client::write_raw(const uint8_t *buf, int32_t size) {
  if (this->can_send() <= 0) {
    return 0;
  }

  int sent = ::sendto(this->m_sockfd, (const char *)buf, size, 0,
                      (SOCKADDR *)&this->m_addr, sizeof(SOCKADDR_IN));
...
void udp_client::_recv(const uint8_t *buf, int32_t size) {
