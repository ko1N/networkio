...
client::client() {
  this->m_client = std::make_shared<networkio::socket::tcp_client>();
}
...
- follow redirects
*/
networkio::http::response client::request_sync(std::string url) {
...
bool client::build_request(std::string &url, std::string &address,
...
client::process_request(const std::string &address,
                        const networkio::http::request &request,
                        int redirects) {
  if (this->m_client == nullptr) {
    return response();
  }
  ...
  while (true) {
    ...
    if (this->m_timeout > 0.0) {
      ...
      if (delta > this->m_timeout) {
        printf("http::client::process_request(): connection to \"%s\" timed "
               "out.\n",
               address.c_str());
        this->m_client->close();
        break;
    ...
    if (rcvd < 0) {
      printf("http::client::process_request(): unable to receive response, "
             "retrying #%d times.\n",
    ...
    case PARSE_SUCCESS: {
      ...
      if (resp.status() >= 300 && resp.status() < 400 &&
          redirects < this->m_follow_redirects &&
        ...
        printf("http::client::process_request(): http %d redirect to \"%s\".\n",
               resp.status(), redirect_url.c_str());

        std::string redirect_address;
        networkio::http::request redirect_request;
        if (!this->build_request(redirect_url, redirect_address,
                                 redirect_request)) {
          printf("http::client::process_request(): unable to build redirect "
                 "request.\n");
          return resp;
        }
        return this->process_request(redirect_address, redirect_request,
                                     redirects++);
      }
