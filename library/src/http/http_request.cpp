...
parser_status
request::from_string(const std::string &req) {
	...
	if (header_tokens.size() < 3) {
		printf("http::request::from_string(): invalid header line in request:\n%s\n\n", req.c_str());
		return PARSE_ERROR;
	}
	...
	if (this->m_header_fields["content-length"] != "") {
		try {
			...
			if (content_length > 0 && this->m_body.length() < (size_t)content_length) {
				// printf("http::request::from_string(): response not finished yet %d < %d.\n",
	...
	// check for form data
	if (this->m_header_fields["content-type"] == "application/x-www-form-urlencoded") {
		// is it ok to override get params here?
		this->m_get_params = this->parse_get_params(this->m_body); // TODO: rename to params
	}
