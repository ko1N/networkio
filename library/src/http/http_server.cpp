...
server_handler::process() {
	...
	while (!this->m_request_complete) {
		...
		switch (result) {
			...
			case PARSE_SUCCESS: {
				...
				this->m_response_bytes_left = this->m_response.length();
			} break;
		}
	}

	int32_t sent = this->m_client->write_raw(this->m_response_buf, this->m_response_bytes_left);
	if (sent < 0) {
	...
	if (this->m_response_bytes_left <= 0) {
		...
		if (this->m_keep_alive_max > 0 && this->m_request_count >= this->m_keep_alive_max) {
			this->m_client->close();
			return false;
		}
	...
	*/
	...
	if (this->m_keep_alive) {
		conn.response().header_fields()["keep-alive"] =
			"timeout=" + std::to_string(this->m_keep_alive_timeout) + ", max=" + std::to_string(this->m_keep_alive_max);
	}
...
parser_status
server_handler::handle_request_get(connection &conn) {
	auto &req = conn.request();
...
parser_status
server_handler::handle_request_post(connection &conn) {
	auto &req = conn.request();
...
endpoint_lock
server::endpoint(const std::string &endpoint) {
	if (endpoint.empty()) {
...
bool
server::process_client(std::shared_ptr<concurrent_server_handler> cl) {
	return cl->process();
}
