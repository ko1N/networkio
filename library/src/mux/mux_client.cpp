...
client::client(std::shared_ptr<networkio::interfaces::client> cl) {
  ...
  this->m_proto =
      std::make_shared<networkio::proto::packet_proto>(this->m_client);
}

client::~client() {}

void client::set_auto_reconnect(int seconds) {
  this->m_auto_reconnect = seconds;
}

int client::get_auto_reconnect() { return this->m_auto_reconnect; }

bool client::run(const std::string &address) {
...
bool client::is_connected() { return this->m_connection_state == 1; }
...
bool client::process() {
  if (this->m_proto == nullptr || !this->m_proto->process()) {
    ...
    if (this->m_auto_reconnect > 0 && this->m_server != "") {
      if ((clock() - this->m_disconnect_time) / CLOCKS_PER_SEC >
          this->m_auto_reconnect) {
...
void client::event(event_type evt, std::function<void(client *)> &&func) {
  this->m_events[evt] = func;
}

bool client::endpoint(
    const hash::hash32_t endpoint,
...
bool client::call(const hash::hash32_t endpoint, const proto::packet *p) {
