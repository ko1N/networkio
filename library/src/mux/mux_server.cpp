...
void
server::event(event_type evt, std::function<void(client *)> &&func) {
	this->m_events[evt] = func;
}

bool
server::endpoint(const hash::hash32_t endpoint, const callback_t &&func) {
...
bool
server::process_client(std::shared_ptr<concurrent_server_handler> _cl) {
