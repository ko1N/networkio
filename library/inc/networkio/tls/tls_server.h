...
public:
  // options
  bool set_certificate(const std::string &cert, const std::string &priv_key,
                       const std::string &priv_key_pwd);
  bool
  set_buffer_size(size_t bufsize = Botan::TLS::Channel::IO_BUF_DEFAULT_SIZE);
  ...
  virtual std::shared_ptr<networkio::interfaces::client>
