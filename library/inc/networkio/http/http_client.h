...
class client {
  ...
  public:
	...
	*/
	...
	void
	set_follow_redirects(int r) {
		this->m_follow_redirects = r;
	}

	networkio::http::response request_sync(std::string url);
	void request_async(std::string url, std::function<void(const networkio::http::response &)> &&callback);
