...
class connection {
  ...
  public:
	...
	inline void
	set_request(const networkio::http::request &r) {
		this->m_request = r;
	}
	...
	inline void
	set_response(const networkio::http::response &r) {
		this->m_response = r;
	}
...
class endpoint {
  ...
  public:
	endpoint &
	get(std::function<void(connection &)> &&func) {
		this->m_get = func;
		return *this;
	}
	...
	endpoint &
	post(std::function<void(connection &)> &&func) {
		this->m_post = func;
		return *this;
	}
...
class server;
class server_handler : public networkio::socket::tcp::concurrent_server_handler {
