...
public:
  ...
  void set_name(const std::string &s) { this->m_name = s; }

  const std::string &value() const { return this->m_value; }

  void set_value(const std::string &s) { this->m_value = s; }

  const std::string &expire_date() const { return this->m_expire_date; }

  void set_expire_date(const std::string &s) { this->m_expire_date = s; }

  int32_t max_age() const { return this->m_max_age; }

  void set_expire_date(int32_t i) { this->m_max_age = i; }

  const std::string &domain() const { return this->m_domain; }

  void set_domain(const std::string &s) { this->m_domain = s; }

  const std::string &path() const { return this->m_path; }

  void set_path(const std::string &s) { this->m_path = s; }

  bool secure() const { return this->m_secure; }

  void set_secure(bool b) { this->m_secure = b; }

  bool http_only() const { return this->m_http_only; }

  void set_http_only(bool b) { this->m_http_only = b; }

  const std::string &same_site() const { return this->m_same_site; }

  void set_same_site(const std::string &s) { this->m_same_site = s; }
...
public:
  virtual void set_body(const std::string &s) {
    this->set_status(200);
    this->m_header_fields["content-length"] = std::to_string(s.length());
    this->m_body = s;
  }

  int32_t status() { return this->m_status; }

  void set_status(int32_t s) {
    this->m_status = s;
    this->m_status_str = status_code_to_str(s);
  }

  std::unordered_map<std::string, response_cookie> &cookies() {
