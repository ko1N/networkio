...
public:
  ...
  void set_type(std::string type) {
    ...
    std::transform(this->m_type.begin(), this->m_type.end(),
                   this->m_type.begin(), ::toupper);
    this->update_header();
  }

  const std::string &uri() const { return this->m_uri; }
  ...
  const std::string &proto() const { return this->m_proto; }
...
protected:
  ...
  parse_get_params(const std::string &s);
  std::unordered_map<std::string, std::string>
  parse_cookies(const std::string &s);
